...
import { ExplanationModal } from "../IzstarDictionary/ExplainationModal";
import { DictionaryItem, starDictionary } from "../IzstarDictionary";
...
export const Izpalace = ({
	...
	const handleStarsClick = (event: React.MouseEvent) => {
		...
		if (target !== event.currentTarget) {
			...
			const dictionaryItem = starDictionary.find(
				(item) => item.keyword === directText
			);
			if (dictionaryItem) {
				setSelectedItem(dictionaryItem);
			}
		}
	};
			...
			>
					...
					{horoscopeNames?.map((item) => (
						<span
							...
							className={classNames({
								[`iztro-palace-${item.scope}-active`]:
									item.show,
							})}
							onClick={
								item.scope
									? () => toggleScope?.(item.scope as Scope)
									: undefined
							}
						>
							{item.name}
				...
				<div className={classNames("iztro-palace-footer")}>
					...
					<div>
						...
						<div
							className={classNames("iztro-palace-dynamic-name")}
						>
							{showDecadalScope && (
					...
					<div>
						<div className={classNames("iztro-palace-rgt24")}>
							<div>
								{showYearlyScope
									? horoscope?.yearly.yearlyDecStar.suiqian12[
											index
									  ]
									: palace.suiqian12}
							</div>
							<div>
								{showYearlyScope
									? horoscope?.yearly.yearlyDecStar
											.jiangqian12[index]
									: palace.jiangqian12}
							</div>
						</div>
						...
						<div
							...
							onMouseLeave={() =>
								setHoverHeavenlyStem?.(undefined)
							}
						>
							<span
								className={classNames({
